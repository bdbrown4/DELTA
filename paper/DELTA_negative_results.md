...
This note is **not validated on any other architecture or standard benchmark.** It is a hypothesis about evaluation paradigms scoped to DELTA, and its highest-value follow-up is cross-architecture + standard-benchmark replication (§6, §7). Adjacent evidence exists: Li et al. (2023) report on *standard* KGC benchmarks that message passing can be replaced by MLPs with little performance loss, attributing performance chiefly to scoring-function and loss-function design — consistent in spirit with "the readout does the work," though their setting does not test our specific trained-readout-vs-soft-traversal contrast.
...
**Independent corroboration.** After completing the experimental arc, we found that components of our conclusion had been reached independently by other routes. On the theory side, Huang, Romero Orth, Ceylan & Barceló (NeurIPS 2023; arXiv:2302.02209) characterize the expressive power of NBFNet-style conditional MPNNs via relational Weisfeiler–Leman tests: the power of these architectures is fully characterized by *query-(source/relation-)conditioned node message passing*, with no appeal to the path view (they show NBFNet's Bellman–Ford path interpretation does not provably hold under non-linearities, and that expressivity is independent of the history function). They do not analyze edge-level message passing; the edge-view implication in this paper comes from our ablations, not their theorem. On the empirical side, Li et al. (ACL 2023; arXiv:2205.10652) find on standard KGC benchmarks that replacing message passing with simple MLPs leaves performance largely intact, attributing performance chiefly to scoring-function and loss-function design. Our 79-phase arc arrived at convergent conclusions — query-conditioning is the lever (§3.4) and the trained scoring head, not propagation, carries the signal (§5) — through controlled ablation on an edge-attention architecture rather than through expressivity theory or MLP substitution. We read this convergence as strengthening the conclusion while diminishing none of the scope caveats above: our contribution is the *controlled, mechanism-level* evidence, not priority on the conclusion.
