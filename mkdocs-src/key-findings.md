...
### 45. Top-k sparse attention at topk=128 perfectly preserves full-attention quality; topk=64 degrades −5.6%; topk=256 OOM
Phase 64: Top-k sparse edge attention tested at 3 budgets on full 63M E_adj at N=5000. **topk=128 test MRR=0.2472**, matching Phase 63 full-softmax on same data (0.2457, Δ=+0.0015, within noise). Quality is fully preserved at 128 neighbors. **topk=64 test MRR=0.2332** (−5.6% vs topk=128; −3.3% on H@10). The quality cliff lies between 64 and 128 neighbors. **topk=256 OOM** after ep25 on the 98GB Blackwell GPU — memory limit is between 128 and 256 at N=5000. Sparse attention provides no epoch-time speedup at this scale (508s/ep for both topk=128 and full-softmax 525s/ep); the bottleneck is upstream E_adj processing, not the attention kernel. The optimal cost/quality point remains Phase 63 Cond B (30M subsample, full softmax, 37,653s). **Sparse attention is validated as a scaling mechanism: for larger N where full E_adj softmax is infeasible, topk=128 preserves quality without accuracy penalty.**
