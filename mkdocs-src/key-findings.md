...
### 42. DELTA vs DistMult convergence speed — real advantage or compute artifact? (UNRESOLVED)
Phase 61: 1L-DELTA converges in fewer gradient steps than DistMult across N=500/1000/2000. At N=500 (enough steps for both to converge), DM test MRR slightly beats DELTA (−0.016). At N=1000/2000 (limited steps), DELTA wins (+0.026/+0.079) — but DistMult was still improving. **Critical confound: at N=2000, DM trained for 60 seconds while DELTA trained for 5,896 seconds (98× more wall-clock compute).** The "sample efficiency" framing is misleading without controlling for compute: per FLOP or per second, DistMult is vastly more efficient. **Phase 61b (DM at N=2000 for 2000 epochs, ~5 min wall time) must determine whether DM reaches ~0.30+ with more training.** If it does, the "advantage" is a compute artifact. If DM plateaus, the GNN provides genuine inductive bias. No conclusions should be cited until 61b is complete.
