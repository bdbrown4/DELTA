...
**The three-paradigm gap — visual explainer:** [ARCHITECTURE_VISUAL.md](./ARCHITECTURE_VISUAL.md) walks through Transformer → GNN → DELTA with an [interactive diagram](https://htmlpreview.github.io/?https://github.com/bdbrown4/DELTA/blob/main/assets/transformer_vs_graph_vs_delta.html). The key insight: GNN edges are passive scalar wires; DELTA edges are first-class computational citizens that attend to each other. That edge-to-edge attention is what produces the Phase 28 +24% noise robustness gap.
